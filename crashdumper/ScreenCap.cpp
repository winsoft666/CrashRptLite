...
BOOL CScreenCapture::TakeDesktopScreenshot(LPCTSTR szSaveToDir,
                                           ScreenshotInfo& ssi,
  ...
  BOOL bTakeScreenshot = CaptureScreenRect(wnd_list, szSaveToDir, nIdStartFrom, fmt, nJpegQuality,
                                           bGrayscale, ssi.m_aMonitors);
...
BOOL CALLBACK CScreenCapture::EnumMonitorsProc(HMONITOR hMonitor,
                                               HDC /*hdcMonitor*/,
                                               LPRECT lprcMonitor,
                                               LPARAM dwData) {
  ...
  for (i = 0; i < (int)psc->m_arcCapture.size(); i++) {
    ...
    if (IntersectRect(&rcIntersect, lprcMonitor, &rc)) {
      BOOL bBitBlt = BitBlt(hCompatDC, rc.left - lprcMonitor->left, rc.top - lprcMonitor->top,
                            rc.Width(), rc.Height(), hDC, rc.left - lprcMonitor->left,
                            rc.top - lprcMonitor->top, SRCCOPY | CAPTUREBLT);
      if (!bBitBlt)
        goto cleanup;
    }
  }
...
BOOL CScreenCapture::PngInit(int nWidth, int nHeight, BOOL bGrayscale, CString sFileName) {
  ...
  m_png_ptr = png_create_write_struct(PNG_LIBPNG_VER_STRING, (png_voidp)NULL, NULL, NULL);
  if (!m_png_ptr)
    return FALSE;
...
BOOL CScreenCapture::JpegInit(int nWidth,
                              int nHeight,
                              BOOL bGrayscale,
                              int nQuality,
                              CString sFileName) {
...
BOOL CScreenCapture::FindWindows(DWORD dwProcessId,
  ...
  if (!bAllProcessWindows) {
    ...
    for (i = 0; i < paWindows->size(); i++) {
      if (((*paWindows)[i].dwExStyle & WS_EX_APPWINDOW) && ((*paWindows)[i].dwStyle & WS_CAPTION) &&
