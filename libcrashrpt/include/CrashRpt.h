...
typedef struct tagCR_CRASH_CALLBACK_INFOW {
  ...
  BOOL
      bContinueExecution;  //!< Whether to terminate the process (the default) or to continue program execution.
...
#define CR_CB_DODEFAULT \
  1  //!< Proceed to the next stages of crash report generation without calling crash callback function.
...
*  \endcode
*
*  \sa CR_CRASH_CALLBACK_INFO, crSetCrashCallback(), crAddFile2(), crAddProperty(), crAddScreenshot(), crAddRegKey()
*/
...
#define CR_INST_STRUCTURED_EXCEPTION_HANDLER \
  0x1  //!< Install SEH handler (deprecated name, use \ref CR_INST_SEH_EXCEPTION_HANDLER instead).
...
#define CR_INST_NEW_OPERATOR_ERROR_HANDLER \
  0x10  //!< Install new operator error handler (VS .NET and later).
#define CR_INST_SECURITY_ERROR_HANDLER \
  0x20  //!< Install security error handler (VS .NET and later).
#define CR_INST_INVALID_PARAMETER_HANDLER \
  0x40                                  //!< Install invalid parameter handler (VS 2005 and later).
...
#define CR_INST_CRT_EXCEPTION_HANDLERS \
  0x1FFE  //!< Install exception handlers for the linked CRT module.

#define CR_INST_NO_GUI \
  0x2000  //!< Do not show GUI, send report silently (use for non-GUI apps only).
...
#define CR_INST_STORE_ZIP_ARCHIVES \
  0x80000  //!< CrashRpt should store both uncompressed error report files and ZIP archives.
#define CR_INST_AUTO_THREAD_HANDLERS \
  0x800000  //!< If this flag is set, installs exception handlers for newly created threads automatically.
...
*             report files are by default stored in <i>%LOCAL_APPDATA%\\CrashReports\\%AppName%_%AppVersion%</i> folder.
...
#define CR_AF_TAKE_ORIGINAL_FILE \
  0  //!< Take the original file (do not copy it to the error report folder).
#define CR_AF_MAKE_FILE_COPY 1  //!< Copy the file to the error report folder.
#define CR_AF_FILE_MUST_EXIST \
  0  //!< Function will fail if file doesn't exist at the moment of function call.
#define CR_AF_MISSING_FILE_OK \
  2  //!< Do not fail if file is missing (assume it will be created later).
#define CR_AF_ALLOW_DELETE \
  4  //!< If this flag is specified, the file will be deletable from context menu of Error Report Details dialog.
...
*  \endcode
*
*  \sa crAddFileW(), crAddFileA(), crAddFile2()
*/

CRASHRPTAPI(int)
crAddFileW(LPCWSTR pszFile, LPCWSTR pszDestFile, LPCWSTR pszDesc, DWORD dwFlags);


CRASHRPTAPI(int)
crAddFileA(LPCSTR pszFile, LPCSTR pszDestFile, LPCSTR pszDesc, DWORD dwFlags);
...
#define CR_AR_ALLOW_DELETE \
  0x1  //!< If this flag is specified, the file will be deletable from context menu of Error Report Details dialog.
